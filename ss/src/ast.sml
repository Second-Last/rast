...
datatype decl =
       ...
       | ExpDec of expname * Arith.ctx * Arith.prop * (tp * pot * tp) * ext
                                                             (* proc f : A |- C *)
       | ExpDef of expname * Arith.ctx * exp * ext           (* proc f = P *)
...
val lookup_expdec : env -> expname -> (Arith.ctx * Arith.prop * (tp * pot * tp)) option
val lookup_expdef : env -> expname -> (Arith.ctx * exp) option
...
val expd_expdec : env -> expname * Arith.arith list -> (Arith.prop * (tp * pot * tp)) option
...
datatype exp =
       ...
       | Cut of exp * pot * tp * exp           (* P [A] Q  or  P [|{p}- A] Q *)
       | Spawn of exp * exp                    (* P || Q *)
       | ExpName of expname * R.arith list (* f, f{...} *)
       ...
       (* existential quantifier ?{phi}. A *)
       | AssertR of R.prop * exp               (* assertR{phi} ; P *)
       | AssumeL of R.prop * exp               (* assumeL{phi} ; P *)

       (* universal quantifier !{phi}. A *)
       | AssumeR of R.prop * exp               (* assumeR{phi} ; P *)
       | AssertL of R.prop * exp               (* assertL{phi} ; P *)
...
datatype decl =
       ...
       | ExpDec of expname * R.ctx * R.prop * (tp * pot * tp) * ext
       | ExpDef of expname * R.ctx * exp * ext
       | Exec of expname * ext
...
fun pp_decl (TpDef(a,vs,R.True,A,_)) = "type " ^ a ^ pp_vars vs ^ " = " ^ pp_tp A
  ...
  | pp_decl (ExpDec(f,vs,con,(A,pot,C),_)) = "proc " ^ f ^ pp_vars vs ^ pp_con con ^ " : " ^ pp_tp A ^ " |" ^ pp_pot pot ^ "- " ^ pp_tp C
  | pp_decl (ExpDef(f,vs,P,_)) = "proc " ^ f ^ pp_vars vs ^ " = " ^ pp_exp P
