...
fun match_contexts env ctx con D D' ext =
...
(* check_exp trace env ctx con D pot P (z,C) = () if ctx ; con ; D |{pot}- P :: (z : C)
 * raises ErrorMsg.Error otherwise
 * requires ctx |- con prop
 *          ctx ; con |= D valid
 *          ctx ; con |= C valid
 *          ctx ; con |= pot nat
...
and fwd trace env ctx con D pot (A.Id(x,y)) (zC as (z,C)) ext =
...
and delay trace env ctx con D pot (A.Delay(t,P)) (z,C) ext =
    ...
    else check_exp' trace env ctx con (TU.decrement_context env ctx con D t ext) pot P
