...
fun resolve env tpctx (A.Plus(choices)) = A.Plus(resolve_choices env tpctx choices)
  | resolve env tpctx (A.With(choices)) = A.With(resolve_choices env tpctx choices)
  | resolve env tpctx (A.Tensor(A,B)) = A.Tensor(resolve env tpctx A, resolve env tpctx B)
  | resolve env tpctx (A.Lolli(A,B)) = A.Lolli(resolve env tpctx A, resolve env tpctx B)
  | resolve env tpctx (A.One) = A.One
  | resolve env tpctx (A.Exists(phi,A)) = A.Exists(phi,resolve env tpctx A)
  | resolve env tpctx (A.Forall(phi,A)) = A.Forall(phi,resolve env tpctx A)
  | resolve env tpctx (A.ExistsNat(v,A)) = A.ExistsNat(v,resolve env tpctx A)
  | resolve env tpctx (A.ForallNat(v,A)) = A.ForallNat(v,resolve env tpctx A)
  | resolve env tpctx (A.ExistsTp(alpha,A)) = A.ExistsTp(alpha, resolve env (alpha::tpctx) A)
  | resolve env tpctx (A.ForallTp(alpha,A)) = A.ForallTp(alpha, resolve env (alpha::tpctx) A)
  | resolve env tpctx (A.PayPot(p,A)) = A.PayPot(p,resolve env tpctx A)
  | resolve env tpctx (A.GetPot(p,A)) = A.GetPot(p,resolve env tpctx A)
  | resolve env tpctx (A.Next(t,A)) = A.Next(t,resolve env tpctx A)
  | resolve env tpctx (A.Dia(A)) = A.Dia(resolve env tpctx A)
  | resolve env tpctx (A.Box(A)) = A.Box(resolve env tpctx A)
  | resolve env tpctx (A.TpName(a,nil,nil)) =
    ...
    then A.TpVar(a)
    else (case A.lookup_tp env a
           of NONE => A.TpVar(a)
            | SOME _ => A.TpName(a,nil,nil))
  | resolve env tpctx (A.TpName(a,As,es)) =
    A.TpName(a, List.map (fn A => resolve env tpctx A) As, es)
  (* A.TpVar should be impossible since it is never generated by the parser *)
and resolve_choices env tpctx nil = nil
  | resolve_choices env tpctx ((l,A)::choices) =
    (l, resolve env tpctx A)::resolve_choices env tpctx choices
...
fun valid_args env tpctx vs (A.Plus(choices)) ext = valid_args_choices env tpctx vs choices ext
  ...
  | valid_args env tpctx vs (A.ExistsTp(alpha,A)) ext = valid_args env (alpha::tpctx) vs A ext
  | valid_args env tpctx vs (A.ForallTp(alpha,A)) ext = valid_args env (alpha::tpctx) vs A ext
...
and tp_name_subtp tpctx ctx con (A.Plus(choices)) env =
  ...
  | tp_name_subtp tpctx ctx con (A.ExistsTp(alpha,A')) env =
    apply_fst (fn A => (A.ExistsTp(alpha,A))) (tp_name_subtp' (alpha::tpctx) ctx con A' env)
  | tp_name_subtp tpctx ctx con (A.ForallTp(alpha,A')) env =
    apply_fst (fn A => (A.ForallTp(alpha,A))) (tp_name_subtp' (alpha::tpctx) ctx con A' env)
...
fun elab_tps env nil = nil
  ...
  | elab_tps env ((decl as A.ExpDec(f,alphas,vs,con,(D,pot,(z,C)),ext))::decls) =
    ...
    let
        ...
        val D' = List.map (fn (x,A) => (x,resolve env alphas A)) D
        val C' = resolve env alphas C
