signature EXEC =
sig
    (* soft errors resulting from approx type checking *)
    exception SoftError
    (* hard errors resulting from bugs in compiler, should never be reached *)
    exception HardError
    (* may raise Fail(msg) *)
    val exec : Ast.env -> Ast.config -> Ast.config
end
...
exception IncorrectIndicesLength (* mismatch in index length *)

exception AssertionFailure (* assertion failed while sending *)

exception AssumptionFailure (* assumption failed while receiving *)

exception ImpossibleReached (* impossible branch reached *)

exception ConstraintFailure (* constraint fail in call *)
