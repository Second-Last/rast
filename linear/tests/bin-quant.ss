...
proc x <- succ{n} <- y =                      % !n ; y : bin{n} |- x : bin{n+1}
  case y ( b0 =>                              % !n ; y : ?{n > 0}. ?{k}. ?{n = 2*k}. bin{k} |- x : bin{n+1}
                  assume y {n > 0} ;          % !n ; !{n > 0} ; y : ?{k}. ?{n = 2*k}. bin{k} |- x : bin{n+1}
                  {d} <- recv y ;             % !n, !d ; !{n > 0} ; y : ?{n = 2*d}. bin{d}   |- x : bin{n+1}
                  assume {n = 2*d} ;          % !n, !d ; !{n > 0}, !{n = 2*d} ; y : bin{d}   |- x : bin{n+1}
                  x.b1 ;                      % !n, !d, !{n > 0}, !{n = 2*d} ; y : bin{d}   |- x : ?{n+1 > 0}. ?{k}. ?{n+1 = 2*k+1}. bin{k}
                  assert x {n+1 > 0} ;        % !n, !d, !{n > 0}, !{n = 2*d} ; y : bin{d}   |- x : ?{k}. ?{n+1 = 2*k+1}. bin{k}
                  send x {d} ;                % !n, !d, !{n > 0}, !{n = 2*d} ; y : bin{d} |- x : ?{n+1 = 2*d+1}. bin{d}
                  assert x {n+1 = 2*d+1} ;    % !n, !k, !{n > 0}, !{n = 2*d} ; y : bin{d} |- x : bin{d}
                  x <- y
                  % to prove here:
                  % !n. !k. n = 2*k => ?_k'. n+1 = 2*_k'+2 /\ k = _k'
                  % issue here: forwarding checked with reflexivity?
         | b1 =>                              % !n ; y : ?{n > 0}. ?{k}. ?{n = 2*k+1}. bin{k} |- x : bin{n+1}
                 assume y {n > 0} ;           % !n ; !{n > 0} ; ?{k}. ?{n = 2*k+1}. bin{k} |- x : bin{n+1}
                 {d} <- recv y ;              % !n, !d ; ?{n = 2*d+1}. bin{d} |- x : bin{n+1}
