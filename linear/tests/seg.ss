...
decl empty{k} : . |- (s : seg{0}{k})
proc s <- empty{k} <- =
       t <- recv s;
       s <- t

decl one{k} : (x : A) |- (s : seg{1}{k})
proc s <- one{k} <- x =
       t <- recv s ;
       t' <- cons{k} <- x t ;
       s <- t'

decl concat{n1}{n2}{k} : (s1 : seg{n1}{n2+k}) (s2 : seg{n2}{k}) |- (s : seg{n1+n2}{k})
...
decl prepend{n}{k} : (x : A) (t : seg{n}{k}) |- (s : seg{n+1}{k})
proc s <- prepend{n}{k} <- x t =
       u <- one{n+k} <- x ;
       s <- concat{1}{n}{k} <- u t

decl append{n}{k} : (x : A) (t : seg{n}{k+1}) |- (s : seg{n+1}{k})
proc s <- append{n}{k} <- x t =
       u <- one{k} <- x ;
       s <- concat{n}{1}{k} <- t u

decl tolist{n} : (s : seg{n}{0}) |- (l : list{n})
proc l <- tolist{n} <- s =
       t <- nil <- ;
       send s t ;
       l <- s
