...
fun pp_decl env (A.TpDef(a,vs,R.True,A,_)) =
  ...
  | pp_decl env (A.ExpDec(f,vs,con,([(x,A)],pot,(z,C)),_)) =
    "proc " ^ f ^ P.pp_vars vs ^ P.pp_con con ^ " : "
    ^ pp_tp env A ^ " |" ^ pp_pot pot ^ "- " ^ pp_tp env C
  | pp_decl env (A.ExpDec(f,vs,con,(nil,pot,(z,C)),_)) =
    "proc " ^ f ^ P.pp_vars vs ^ P.pp_con con ^ " : "
    ^ "." ^ " |" ^ pp_pot pot ^ "- " ^ pp_tp env C
  | pp_decl env (A.ExpDef(f,vs,(_,P,_),_)) = pp_exp_after env 0 ("proc " ^ f ^ P.pp_vars vs ^ " = ") P
