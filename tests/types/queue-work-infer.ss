...
type queue{n} = &{enq : &{b0 : <{_a*n+_aa}| queue{n+1}, b1 : <{_b*n+_bb}| queue{n+1}},
                  deq : <{_c}| +{none : ?{n = 0}. 1,
                                some : ?{n > 0}. +{b0 : queue{n-1}, b1 : queue{n-1}}}}

proc bit0{n} : queue{n} |{_d}- queue{n+1}
proc bit1{n} : queue{n} |{_e}- queue{n+1}
proc empty : . |{_f}- queue{0}
